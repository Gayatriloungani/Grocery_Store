...
def insert_new_product(connection, product):
    ...
    # Check if the uom_id exists in the uom table
    cursor.execute("SELECT COUNT(*) FROM uom WHERE uom_id = %s", (product['uom_id'],))
    if cursor.fetchone()[0] == 0:
        return f"Error: uom_id {product['uom_id']} does not exist in the uom table."
